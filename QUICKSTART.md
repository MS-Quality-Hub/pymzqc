...
```python
    ...
    with open("nameOfYourFile.mzQC", "r") as file:
        my_run_qualities = qc.JsonSerialisable.from_json(file)
```
...
## Store
```python
    inmem_file = qc.JsonSerialisable.to_json(mzqc, readability=1)
...
## Validation
The previous sections excusively used the `MZQCFile` submodule, which is what you will need for most tasks. 
The submodule is therefore optimised to be quick and to-the-point for that purpose.
Validation-tasks are unfortunately ill-suited for conscise 'few-liners'.
If you want to make use of the validation capabilities of pymzqc, a look at the offlinevalidator code and its documentation might be a good entrypoint.