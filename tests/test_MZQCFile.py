...
infi = qc.InputFile(name="file.raw", location="file:///dev/null",
                    ...
                    file_properties=[qc.CvParameter(accession="MS:1000747",
                                                    name="completion time",
...
meta = qc.MetaDataParameters(input_files=[infi], analysis_software=[anso], label="test_metadata")
...
mzqc = qc.MzQcFile(version="1.0.0",
                   ...
                   run_qualities=[rq], set_qualities=[sq],
...
class TestSerialisation:
    ...
    def test_DateTime(self):
        try:
            zqc = qc.MzQcFile(version="0.1.0", creation_date=datetime.now().isoformat(), run_qualities=[], set_qualities=[], controlled_vocabularies=[])
...
class TestDeserialisation:
    def test_ControlledVocabulary(self):
        ...
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(cv)), qc.ControlledVocabulary)

    def test_CvParameter(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(cvt)) == cvt
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(cvt)), qc.CvParameter)

    def test_AnalysisSoftware(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(anso)) == anso
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(anso)), qc.AnalysisSoftware)

    def test_InputFile(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(infi)) == infi
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(infi)), qc.InputFile)

    def test_MetaDataParameters(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(meta)) == meta
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(meta)), qc.MetaDataParameters)

    def test_QualityMetric(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(qm)) == qm
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(qm)), qc.QualityMetric)
    ...
    def test_RunQuality(self):
        sdrq = (qc.RunQuality)(**qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(rq)).__dict__)
        assert  sdrq == rq
        assert isinstance(sdrq,qc.RunQuality)

    def test_SetQuality(self):
        assert qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(sq)) == sq
        assert isinstance(qc.JsonSerialisable.from_json(qc.JsonSerialisable.to_json(sq)), qc.SetQuality)
